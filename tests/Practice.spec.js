...
test("Practice", async ({ page }) => {
    ...
    //Handling the webTable /Pagination
    ...
    console.log('Total number of columns are ', await columns.count)
    await expect(columns.count).toBe(4)
    const rows = await table.locator('tbody tr')
    console.log('Total number of columns are ', await rows.count)

    // select checkbox for product 4

    const matchsRow = rows.filter({
        has: page.locator(td), //not needed
        hasText: 'Product 4'
    })
    await matchsRow.locator('input').check()

    // select multiple checkbox for different products
    //first creat a function and then we can call that function

    await matchsRows(rows, page, 'Product 1')
    await matchsRows(rows, page, 'Product 3')
    await matchsRows(rows, page, 'Product 5')



    // Print all the data from each row

    for (let i = 0; i, await rows.count; i++) {
        const row = rows.nth(i)
        const column = row.locator('td')
        for (let j = 0; j < await columns.count - 1; j++) {
            console.log(await column.nth(j).textContent())
        }

    }

    //Print all the data from each row from each page

    const pages = page.loactor('.pagination li a')
    const numPages = await pages.count()
    ...
    //Directly fill date
    await page.fill("#datepicker", '06/15/2025')
    ...
    // Mouse hover action
             // store a element in a const
             //then await element.hover()
  
     // Right click and double click 
           //await element.click({element: 'right'})
           //await element.dblclick()

    // Drag and drop
        // we will have src and trg element
         //Approach-1
...
//Keyboard actions
      ...
      await page.keyboard.down('Tab')  // beacause pressing single key     
      await page.keyboard.up('Tab')
      await page.keyboard.press('Control+V')


 //Uploading single file     
     //use url https://www.foundit.in/
       await  page.locator("locator where we need to upload a file").setInputFiles('path of file')
    
   //Uploading 2 files together
    await  page.locator("locator where we need to upload a file").setInputFiles(['path of file 1','path of file 2'])

  // Removing uploaded files
  await  page.locator("locator where we need to upload a file").setInputFiles([])  
...
async function matchsRows(rows, page, productName) {
    const matchsProduct = rows.filter({
        has: page.locator(td),
        hasText: productName
    })
    await matchsProduct.locator('input').check()
}
